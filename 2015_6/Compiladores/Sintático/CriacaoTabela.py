tabela = []
with open('criacao_tabela.txt', 'r') as file:
    head = file.readline().rstrip().split(',')
    for row in file:
        cont = 0
        for colunm in row.split(','):
            if colunm.isdigit():
                tabela.append(row.split(',')[0] + '|' + head[cont] + '|' + colunm + '\n')
                print(f"{row.split(',')[0]}|{head[cont]}|{colunm}")
            cont += 1

with open('tabela_sintatica.txt', 'w')as file:
    file.writelines(tabela)
'''
step = {
    ...
    '11': '<LOOP>|abre|parenteses|<ARGUMENTO_NUMERICO>|virgula|<OP_MATEMATICA>|<ARGUMENTO_NUMERICO>|fecha_parenteses|dois_pontos|<CODIGO>|fim_repeticao|final_linha',
